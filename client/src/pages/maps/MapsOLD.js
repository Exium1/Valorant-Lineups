...
function Maps(props) {
	...
	if (!props.match.params.agentName) {
		...
		return <Redirect to={`/maps/${mapName.toLowerCase()}/agents/${agentNames[0].toLowerCase()}`} />;
	...
	function selectAgent(agentIndex) {
		...
		for (i = 0; i < tabButtons.length; i++) {
			...
			if (i === agentIndex) tabButton.className += " selected-tab";
				...
				tabButton.className = tabButton.className.replace(" selected-tab", "");
			}
		}
	}
	...
	/*
        ...
        - maps/ascent/agent/astra/site/a/plant/3?defense=true
    ...
    */
	...
	return (
		<>
			<img className="map-bg-image" src={`/../assets/images/maps/${mapName}/LoadingScreen.png`} alt="" />
				...
				<div className="tabs-container">
					...
					<ul className="tabs-buttons">
						{agentNames.map((agent) => {
							if (agentNames.indexOf(agent) === agentNames.indexOf(agentName))
								return (
									<li className={"tab-button selected-tab"} key={agentNames.indexOf(agent)}>
										<Link to={`/maps/${mapName.toLowerCase()}/agents/${agent.toLowerCase()}`}>
											<button>
								...
								return (
									<li className="tab-button" key={agentNames.indexOf(agent)}>
										<Link to={`/maps/${mapName.toLowerCase()}/agents/${agent.toLowerCase()}`}>
											<button>
												<img src={`/../assets/images/agents/${agent}Icon.png`} alt="" />
												{agent}
